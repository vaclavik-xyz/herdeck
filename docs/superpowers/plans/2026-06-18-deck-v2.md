...
## Task 9: cairosvg dep + wire the tick partial render
...
(The agent slug map `DEFAULT_AGENT_SLUGS` is defined in `icons.py` in Task 7.)

- [ ] **Step 1: Make the tick use the driver's partial render when present**
...
- [ ] **Step 3: Update `App.handle_tick` in `src/herdeck/app.py`**
...
- [ ] **Step 4: Update `pyproject.toml` `[deck]` extra**

```toml
deck = ["strmdck", "pillow>=10", "cairosvg", "deepdiff", "python-dotenv", "hidapi"]
```
(Note: `cairosvg` needs the system Cairo lib — `brew install cairo` on macOS. It is
lazy-imported only when building icons, so tests don't require it.)
...
```bash
git add src/herdeck/app.py tests/test_app.py pyproject.toml
git commit -m "feat: cairosvg dep, partial spinner render on tick"
```

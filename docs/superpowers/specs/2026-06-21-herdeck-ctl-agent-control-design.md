...
**Důsledky:** remote/auth/multi-server zadarmo; **identická sémantika s deckem** (jeden zdroj pravdy — profily i wire encoder se sdílejí); `wait` z event streamu; funguje dnes (běží launchd bridge na `100.x.y.z:8788`). Cena: jednorázový příkaz zaplatí ~vteřinu na connect + první snapshot.

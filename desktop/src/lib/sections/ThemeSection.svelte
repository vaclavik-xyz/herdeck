<script lang="ts">
  ...
  const accents = $derived((getAt(payload, "base", SEC, "server_accents") as string[]) ?? DEFAULT_SERVER_ACCENTS);
  ...
  function setBaseAccents(state: ListFieldState, list: string[]): void {
    payload = setListField(payload, "base", SEC, "server_accents", state, list);
    onChange();
  }
  ...
  function inheritedAccents(): string[] {
    const v = inheritedFor(payload, prof, SEC, "server_accents");
    return Array.isArray(v) ? v as string[] : DEFAULT_SERVER_ACCENTS;
  }
  function accentHint(): string { const v = inheritedAccents(); return v.length > 0 ? v.join(" · ") : lm.none; }

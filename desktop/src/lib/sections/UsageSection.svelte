...
{#if overlay}
  ...
  <OverrideField label="refresh_secs" help={HELP.refresh_secs} state={scState("refresh_secs")} inheritedDisplay={hint("refresh_secs")} onstate={(s) => setScState("refresh_secs", s)}>
    <NumberField label="" int value={Number(scValue("refresh_secs"))} onchange={(v) => setSc("refresh_secs", v ?? 300)} />
  </OverrideField>
...
{:else}
  ...
  <NumberField label="refresh_secs" help={HELP.refresh_secs} int value={refreshSecs} onchange={(v) => set("refresh_secs", v ?? 300)} />

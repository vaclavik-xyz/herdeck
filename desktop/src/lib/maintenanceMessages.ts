...
export const MAINTENANCE_MESSAGES = defineMessages({
  en: {
    ...
    config_error: "The config does not load: {error}",
    config_error_hint: "The deck shows this error instead of agents. Fix it in Settings (a runtime service reads tokens from env, token_file or the keychain) — the deck recovers without a restart.",
    ...
    hooks_hint: "Hooks in Claude Code and Codex (a plugin in OpenCode) on this bridge's machine report running subagents to the deck (⑂N badge).",
    hooks_agent_claude: "Claude Code",
    hooks_agent_codex: "Codex",
    hooks_agent_opencode: "OpenCode",
    hooks_on: "On",
    hooks_off: "Off",
    ...
    hooks_install_plugin_confirm: "Install the {agent} subagent plugin on {id}? herdeck writes {file} and leaves every other plugin alone; an older copy of that file is backed up first.",
    hooks_uninstall_plugin_confirm: "Remove the {agent} subagent plugin from {id}? Only {file} is deleted, after a backup next to it; other plugins stay.",
  ...
  cs: {
    ...
    config_error: "Config nejde načíst: {error}",
    config_error_hint: "Deck místo agentů ukazuje tuto chybu. Oprav ji v Nastavení (služba runtime čte tokeny z env, token_file nebo klíčenky) — deck se vzpamatuje bez restartu.",
    ...
    hooks_hint: "Hooky v Claude Code a Codexu (v OpenCode plugin) na stroji tohoto bridge hlásí decku běžící subagenty (odznak ⑂N).",
    hooks_agent_claude: "Claude Code",
    hooks_agent_codex: "Codex",
    hooks_agent_opencode: "OpenCode",
    hooks_on: "Zapnuto",
    ...
    hooks_install_plugin_confirm: "Nainstalovat plugin pro subagenty {agent} na {id}? herdeck zapíše {file} a ostatní pluginy nechá být; starší verzi souboru předtím zazálohuje.",
    hooks_uninstall_plugin_confirm: "Odebrat plugin pro subagenty {agent} z {id}? Smaže se jen {file} (vedle zůstane záloha); ostatní pluginy zůstanou.",

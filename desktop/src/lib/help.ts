...
export const FIELD_HELP: Record<Lang, Record<string, SectionHelp>> = {
  en: {
    ...
    view: {
      ...
      tile_icon: "What the tile's logo box shows: agent = agent logo, project = project favicon (letter badge if none), both = logo plus project badge.",
      project_icons: "Repo name → icon file on the deck machine (PNG/ICO ≤256 KiB; SVG only in source installs); overrides the favicon found in the repo.",
  ...
  cs: {
    ...
    view: {
      ...
      tile_icon: "Co ukazuje rámeček loga: agent = logo agenta, project = favicon projektu (bez něj písmeno), both = logo agenta s odznakem projektu.",
      project_icons: "Název repa → soubor ikony na stroji s deckem (PNG/ICO ≤256 KiB; SVG jen při instalaci ze zdrojů); přebije favicon nalezený v repu.",

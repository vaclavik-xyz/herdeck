...
def test_resolve_source_kind_does_not_bypass_missing_exact_socket_override(
    monkeypatch, tmp_path
):
    config_path = tmp_path / "config.toml"
    local_path = tmp_path / "local.toml"
    ...
    review_socket.parent.mkdir(parents=True)
    review_socket.touch()
    missing_override = tmp_path / "missing.sock"
    local_path.write_text('[local]\nherdr_sessions = ["review"]\n')
    monkeypatch.setenv("HOME", str(tmp_path))
    monkeypatch.setenv("HERDR_SOCKET", str(missing_override))
    monkeypatch.delenv("HERDR_SOCKET_PATH", raising=False)
    monkeypatch.delenv("HERDR_SESSION", raising=False)
    monkeypatch.setattr(
        server_module,
    ...
    assert server_module._resolve_source_kind() == ("mock", "local_unavailable")


def test_resolve_source_kind_does_not_bypass_missing_env_session(
    monkeypatch, tmp_path
):
    config_path = tmp_path / "config.toml"
    local_path = tmp_path / "local.toml"
    ...
    monkeypatch.delenv("HERDR_SOCKET", raising=False)
    monkeypatch.delenv("HERDR_SOCKET_PATH", raising=False)
    monkeypatch.setenv("HERDR_SESSION", "missing")
    monkeypatch.setattr(
        server_module,
        "_default_config_paths",
        lambda: (str(config_path), str(local_path)),
